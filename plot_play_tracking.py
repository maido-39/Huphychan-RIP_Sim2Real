...
def run(cfg: PlotPlayTrackingConfig) -> Path:
  ...
  axes[0].plot(t, data["cylinder_q_post_deg"], label="q_actual_deg")
  axes[0].plot(t, data["cylinder_q_pre_deg"], label="q_pre_deg", alpha=0.35)
  ...
  axes[1].plot(t, data["cylinder_qd_post_deg_s"], label="q_actual_vel_deg_s")
  axes[1].axhline(0.0, linewidth=1.0, color="black")
  axes[1].set_ylabel("velocity [deg/s]")
  axes[1].set_title("Target velocity vs actual velocity")
  axes[1].grid(True)
  axes[1].legend()

  axes[2].plot(t, data["tracking_error_post_deg"], label="q_target - q_actual")
  axes[2].plot(t, data["tracking_error_pre_deg"], label="pre-step error", alpha=0.45)
  ...
  axes[3].set_title("Revolute 3 actuator torque")
  axes[3].grid(True)
  axes[3].legend()
  ...
  if "reward" in data:
    ...
    axes[4].plot(t, reward / denom, label="reward normalized", alpha=0.45)
  axes[4].axhline(1.0, linewidth=1.0, linestyle=":")
  axes[4].axhline(-1.0, linewidth=1.0, linestyle=":")
  axes[4].axhline(0.0, linewidth=1.0, color="black")
  axes[4].set_ylabel("normalized")
  axes[4].set_xlabel("simulation time [s]")
  axes[4].set_title("Policy command")
  axes[4].grid(True)
  axes[4].legend()
