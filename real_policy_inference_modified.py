...
- action 의미:
  ...
  -> scaled_target_angle_rad = action * 20deg
  -> target_angle_rad = rate/accel limiter를 통과한 실제 송신 목표각
- 현재 학습 설정 기준 주기:
  -> timestep = 0.005
  -> decimation = 4
...
TARGET_DELTA_LIMIT_RAD = TARGET_RATE_LIMIT_RAD_S * POLICY_DT
TARGET_DELTA_CHANGE_LIMIT_RAD = TARGET_ACCEL_LIMIT_RAD_S2 * POLICY_DT * POLICY_DT
CAN_FRAME_FORMAT = "=IB3x8s"
CAN_FRAME_SIZE = struct.calcsize(CAN_FRAME_FORMAT)

# inverse_env_cfg.py의 observations["actor"] 항목 순서와 맞춘 slice.
# ObservationManager는 history를 전체 obs가 아니라 항목별로 쌓아 concat한다.
_OBS_TERM_SLICES = (
  slice(0, 1),  # cylinder_angle
  slice(1, 3),  # cylinder_angle_periodic
  slice(3, 5),  # pole_angle
  slice(5, 7),  # joint_vel
...
class InverseRealPolicy:
  ...
  def _denormalize_cylinder_angle(self, cylinder_angle_rad: float) -> float:
    """Convert internal simulator-frame cylinder angle to physical motor degrees."""
    return (
      cylinder_angle_rad * 180.0 / math.pi / self.cfg.cylinder_sign
  ...
  def _apply_target_limiter(
    ...
    current_cylinder_rad: float,
  ) -> float:
    """Match inverse_env_cfg.RateLimitedJointPositionAction for real inference."""
    if self._limited_target_rad is None:
      self._limited_target_rad = current_cylinder_rad
      self._target_delta_rad = 0.0
  ...
  def build_policy_input(self, meas: RealMeasurement) -> torch.Tensor:
    """Build the 16D policy input using ObservationManager's term-wise history."""
  ...
  def infer(self, meas: RealMeasurement) -> dict[str, float]:
    ...
    current_cylinder_rad, _ = self._normalize_angles(meas)
    ...
    limited_target_rad = self._apply_target_limiter(
      scaled_target_rad,
      current_cylinder_rad,
    )
    scaled_target_deg = self._denormalize_cylinder_angle(scaled_target_rad)
    target_angle_deg = self._denormalize_cylinder_angle(limited_target_rad)

    return {
      "raw_policy_action": raw_action,
      "policy_action": action,
      "scaled_target_angle_rad": scaled_target_rad,
      "scaled_target_angle_deg": scaled_target_deg,
      "target_angle_rad": limited_target_rad,
      "target_angle_deg": target_angle_deg,
      "target_delta_deg": self._target_delta_rad * 180.0 / math.pi,
    }
...
def _print_example(result: dict[str, float]) -> None:
  print("raw_policy_action :", f"{result['raw_policy_action']:.6f}")
  print("policy_action     :", f"{result['policy_action']:.6f}")
  print("scaled_target_deg :", f"{result['scaled_target_angle_deg']:.6f}")
  print("target_delta_deg  :", f"{result['target_delta_deg']:.6f}")
...
def _run_can_loop(cfg: MainConfig) -> None:
  ...
  last_policy_update_s = -math.inf
  latest_result: dict[str, float] | None = None
  ...
  try:
    ...
    while True:
      ...
      policy_updated = latest_result is None or now_s - last_policy_update_s >= POLICY_DT
      if policy_updated:
        latest_result = policy.infer(meas)
        last_policy_update_s = now_s
      ...
      print(
        "tick={}  cyl={:+8.3f} deg  pole={:+8.3f} deg  cmd={:+8.3f} deg  "
        "scaled={:+8.3f} deg  raw={:+6.3f}  act={:+6.3f}".format(
          int(policy_updated),
          meas.cylinder_angle_deg,
          meas.pole_angle_deg,
          result["target_angle_deg"],
          result["scaled_target_angle_deg"],
          result["raw_policy_action"],
