...
def _with_mit(
  bus, motor_id: int, protocol: str, protocol_order, assume_yes: bool, action
):
  ...
  if original_protocol != "mit":
    ...
    if not ok:
      print(
        f"WARNING: failed to switch back to {original_protocol}; motor left in {cur_protocol}."
      )
...
def main() -> int:
  ap = argparse.ArgumentParser(
    description="RobStride minimal setup: CAN ID / protocol / zero"
  )
  ap.add_argument("--interface", default="socketcan")
  ap.add_argument("--channel", default="can0")
  ap.add_argument(
    "--motor-id", type=int, default=None, help="known motor id; omit to auto-scan"
  )
      ...
      if choice in ("0", ""):
        ...
        if not ok:
          ...
          try:
            ...
            msg = can.Message(
              arbitration_id=int(motor_id), data=data, is_extended_id=False
            )
            bus.send(msg)
            ...
            if reply is not None:
              ...
              if abs(current_pos_rad) < 0.05:
                ...
                print(
                  "⚠️ 경고: 영점 세팅 완료 판정은 떴으나, 현재 읽힌 위치가 0.0에서 벗어나 있습니다."
                )
            else:
