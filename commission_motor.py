...
def _matches_private_reply(msg: Any, *, motor_id: int, expected_comm: Optional[int] = None) -> bool:
...
def switch_canopen_to_private(bus: Any, motor_id: int, *, host_id: int = 0x01, timeout_s: float = 0.5) -> bool:
...
def switch_private_to_mit(bus: Any, motor_id: int, *, host_id: int = 0x01, timeout_s: float = 0.5) -> bool:
    arb_id = make_ext_id(0x19, int(host_id), int(motor_id))
...
def change_motor_id(bus: Any, old_motor_id: int, new_motor_id: int, *, timeout_s: float = 0.5) -> bool:
    data = [0xFF] * 8
...
def _find_protocol_for_id(bus: Any, motor_id: int, protocol_order: tuple[str, ...]) -> Optional[str]:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_wizard.add_argument("--motor-id", type=int, default=None, help="Known motor ID; if omitted, auto-detect")
    ...
    p_wizard.add_argument("--motor-model", choices=tuple(MOTOR_MODEL_ID_MAP.keys()), default=None)
    p_wizard.add_argument("--new-id", type=int, default=None, help="Target final motor ID")
