...
def _matches_motor_state_frame(msg, motor_id: int) -> bool:
  """상태 피드백 전용 매칭: 응답은 0xFD로 오고, 실제 모터ID는 payload 첫 바이트에 담긴다."""
  if bool(getattr(msg, "is_extended_id", False)):
    return False
  if int(msg.arbitration_id) != 0xFD:
    return False
  data = bytes(msg.data)
  if len(data) < 1:
    return False
  return data[0] == int(motor_id)
...
class RobotState:
  ...
  pendulum_velocity_deg_s: Optional[float]
...
class MotorStateReader:
  ...
  def _poll_loop(self) -> None:
    ...
    while self._running:
      t0 = time.monotonic()
      msg = can.Message(
        arbitration_id=int(self.motor_id), data=data, is_extended_id=False
      )
      try:
        self._bus.send(msg)
        ...
        continue
      if reply is not None and _matches_motor_state_frame(
        reply, motor_id=self.motor_id
      ):
        self._update(reply)
  ...
  def _passive_loop(self) -> None:
    while self._running:
      ...
      if msg is None:
        continue
      if not _matches_motor_state_frame(msg, motor_id=self.motor_id):
        continue
      self._update(msg)
...
class RobotStateReader:
  ...
  def __init__(
    ...
    self.pole_velocity_estimator = PoleVelocityEstimator(
      ema_alpha=0.5,
    )
    self._last_encoder_timestamp: Optional[float] = None
    self._last_pole_velocity_deg_s: float = 0.0
  ...
  def get_state(self) -> RobotState:
    ...
    pendulum_angle_deg: Optional[float] = None
    pendulum_velocity_deg_s: Optional[float] = None

    if enc is not None:
      pendulum_angle_deg = enc.angle_deg

      if (
        self._last_encoder_timestamp is None
        or enc.timestamp != self._last_encoder_timestamp
      ):
        self._last_pole_velocity_deg_s = self.pole_velocity_estimator.update(
          enc.angle_deg,
          enc.timestamp,
        )
        self._last_encoder_timestamp = enc.timestamp

      pendulum_velocity_deg_s = self._last_pole_velocity_deg_s

    return RobotState(
      pendulum_angle_deg=pendulum_angle_deg,
      pendulum_velocity_deg_s=pendulum_velocity_deg_s,
...
def _demo() -> int:
  ...
  ap.add_argument("--vmax", type=float, default=33.0)
  ap.add_argument("--tmax", type=float, default=17.0)
  ...
  print(
    f"[알림] motor_mode={args.motor_mode}, motor_id={args.motor_id}, "
  ...
  try:
    while True:
      ...
      motor_fresh = (
        ""
        if motor is None
        else (" (stale!)" if motor.timestamp == last_motor_ts else "")
      )
      enc_fresh = (
        "" if enc is None else (" (stale!)" if enc.timestamp == last_enc_ts else "")
      )
      ...
      print(
        ...
        f"pendulum_vel={_fmt(state.pendulum_velocity_deg_s, 'deg/s')}   "
